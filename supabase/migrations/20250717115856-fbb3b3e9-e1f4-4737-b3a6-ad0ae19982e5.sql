-- Core Database Schema Enhancements for Satguru Engravures ERP (Fixed)
...
-- Create optimized stock summary view (fixed column names)
...
SELECT 
  ...
  CASE 
    ...
    ELSE 'NORMAL'
  END as stock_status,
  -- Calculate recent consumption (last 30 days) - using correct column name 'date'
  COALESCE((
    SELECT SUM(qty_issued)
    FROM public.satguru_issue_log il
    WHERE il.item_code = s.item_code
      AND il.date >= CURRENT_DATE - INTERVAL '30 days'
  ), 0) as consumption_30_days,
  -- Calculate recent receipts (last 30 days) - using correct column name 'date'
  COALESCE((
    SELECT SUM(qty_received)
    FROM public.satguru_grn_log gl
    WHERE gl.item_code = s.item_code
      AND gl.date >= CURRENT_DATE - INTERVAL '30 days'
...
-- Stock movement analysis function (fixed column names)
...
BEGIN
  ...
  WITH stock_movements AS (
    SELECT 
      ...
      COALESCE(SUM(CASE WHEN gl.date >= start_date THEN gl.qty_received ELSE 0 END), 0) as received,
      COALESCE(SUM(CASE WHEN il.date >= start_date THEN il.qty_issued ELSE 0 END), 0) as issued
...
-- Indexes for better query performance (using correct column names)
...
ON public.satguru_issue_log(item_code, date);
