...
-- Mark all existing records as legacy data (this handles corrupted dates)
...
-- Enhanced stock summary view with data source awareness and financial year logic
DROP VIEW IF EXISTS satguru_stock_summary_view;

CREATE VIEW satguru_stock_summary_view AS
WITH operational_data AS (
  -- Calculate stock with financial year awareness and legacy data isolation
  SELECT 
    im.item_code,
    im.item_name,
    ...
    -- Opening stock (as of Mar 31, 2025 - end of FY 2024-25)
    COALESCE(opening_stock.opening_stock, 0) as opening_stock,
    
    -- Legacy baseline: All legacy GRNs and Issues (ignore corrupted dates)
    COALESCE(legacy_grns.total_grns, 0) as legacy_grns,
    COALESCE(legacy_issues.total_issues, 0) as legacy_issues,
    ...
    -- Operational transactions (post July 19, 2025 - with proper date validation)
    ...
    -- Track data quality for corrupted legacy dates
    CASE 
  ...
  -- Legacy GRNs (all legacy data, ignore corrupted dates)
  LEFT JOIN (
    SELECT 
      item_code, 
  ...
  -- Legacy Issues (all legacy data, ignore corrupted dates)
  LEFT JOIN (
    SELECT 
      item_code, 
  ...
  -- Operational GRNs (post cutoff, with proper date validation)
  LEFT JOIN (
    SELECT 
      item_code, 
  ...
  -- Operational Issues (post cutoff, with proper date validation)
  LEFT JOIN (
    SELECT 
      item_code, 
...
SELECT 
  *,
  ...
  -- Total GRNs and Issues for compatibility (separate legacy and operational)
  (legacy_grns + operational_grns) as total_grns,
  (legacy_issues + operational_issues) as total_issues
  
FROM operational_data;

-- Create validation function for new entries (post July 19, 2025)
...
BEGIN
  ...
  IF NEW.date >= get_operational_cutoff_date() THEN
    ...
    -- For issues, validate stock availability
...
-- Verification query to check implementation
SELECT 
