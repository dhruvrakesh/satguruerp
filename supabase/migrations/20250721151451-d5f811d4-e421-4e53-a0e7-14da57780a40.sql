-- FINAL FIX: Create correct view that calculates directly from transactions
...
SELECT 
    ...
    0 as reorder_level, -- Default since reorder_level doesn't exist in satguru_item_master
    ...
    CASE 
        ...
        WHEN (COALESCE(opening.opening_stock, 0) + COALESCE(grns.total_grns, 0) - COALESCE(issues.total_issues, 0)) <= 50 THEN 'low_stock' -- Using 50 as default threshold
        WHEN (COALESCE(opening.opening_stock, 0) + COALESCE(grns.total_grns, 0) - COALESCE(issues.total_issues, 0)) > 1000 THEN 'overstock' -- Using 1000 as default threshold
        ELSE 'normal'
    END as stock_status
