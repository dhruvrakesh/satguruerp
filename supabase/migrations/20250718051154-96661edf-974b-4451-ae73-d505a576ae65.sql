-- Data Migration with correct column names
...
-- Populate substrate_catalog with correct column names
...
('BOPP Film 20 Micron', 'FILM', 1000, 20, 'Supplier A', 85.00),
('PET Film 12 Micron', 'FILM', 800, 12, 'Supplier B', 120.00),
('PE Film 25 Micron', 'FILM', 1200, 25, 'Supplier A', 75.00),
('Paper 80 GSM', 'PAPER', 900, 80, 'Paper Mills Ltd', 45.00),
('Aluminum Foil 9 Micron', 'FOIL', 600, 9, 'Foil Corp', 180.00),
('CPP Film 15 Micron', 'FILM', 1100, 15, 'Supplier C', 95.00)
ON CONFLICT (substrate_name) DO NOTHING;

-- Add substrates from existing orders
INSERT INTO public.substrate_catalog (
  substrate_name,
  substrate_type,
  width_mm,
...
SELECT DISTINCT
  substrate as substrate_name,
  'FILM' as substrate_type,
  COALESCE(reel_width_mm, 1000) as width_mm,
  'Legacy Supplier' as supplier
FROM public.orders_dashboard_se
WHERE substrate IS NOT NULL 
  AND substrate != ''
  AND NOT EXISTS (
    SELECT 1 FROM public.substrate_catalog 
    WHERE substrate_name = orders_dashboard_se.substrate
  );
...
-- Now create the enhanced components and update hooks
-- This marks completion of Phase 1: Data Migration & Population