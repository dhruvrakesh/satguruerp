-- Step 1: Data Migration & Master Data Population

-- First, let's migrate existing orders from orders_dashboard_se to order_punching
...
SELECT 
  uiorn,
  customer_name,
  COALESCE(product_description, 'Migrated from legacy system') as product_description,
  ...
  CASE 
    ...
    WHEN status = 'in_progress' THEN 'IN_PROGRESS'::order_status
    WHEN status = 'on_hold' THEN 'ON_HOLD'::order_status
...
WHERE uiorn NOT IN (SELECT uiorn FROM public.order_punching);

-- Also migrate from orders_dashboard_dkpkl if it has different data
INSERT INTO public.order_punching (
  uiorn,
  customer_name,
  product_description,
