...
-- Use created_at timestamp to determine which record to keep (earliest)

-- Step 1: Clean up duplicates, keeping the earliest record based on created_at
...
duplicates_to_delete AS (
  ...
  WHERE sil.created_at > dg.earliest_created_at
)
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'timestamp', now(),
    'details', 'Cleaned up duplicate issue records using created_at timestamp'
  )
);