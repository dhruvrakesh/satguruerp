...
-- Phase 2: Emergency Database Schema Fix & Data Cleanup
-- Step 1: Check if transaction_type column exists and add it if missing
DO $$ 
BEGIN
...
-- Step 2: Check if upload_source column exists and add it if missing
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'satguru_grn_log' 
                   AND column_name = 'upload_source') THEN
        ALTER TABLE public.satguru_grn_log 
        ADD COLUMN upload_source TEXT;
    END IF;
END $$;

-- Step 3: Identify and mark opening stock records consistently
UPDATE public.satguru_grn_log 
SET 
  upload_source = 'OPENING_STOCK',
  vendor = 'Opening Stock',
  transaction_type = 'OPENING_STOCK'
WHERE vendor = 'Opening Stock' 
   OR remarks ILIKE '%opening stock%'
   OR remarks ILIKE '%opening balance%';

-- Step 4: Clean duplicate opening stock entries (412 → 206 records)
-- Keep only the earliest record per item_code for opening stock
...
-- Step 5: Set transaction types for all other records
UPDATE public.satguru_grn_log 
...
CREATE INDEX IF NOT EXISTS idx_grn_log_vendor ON public.satguru_grn_log(vendor);

-- Step 7: Prevent future opening stock duplicates
...
-- Step 8: Update stock table to sync with cleaned GRN data
...
-- Step 9: Verification queries to confirm cleanup
SELECT 
...
-- Final verification: Opening stock should now have exactly 206 records
SELECT 
