-- EMERGENCY FIX: Replace the view to calculate directly from transactions, bypassing corrupted satguru_stock table
...
SELECT 
    ...
    -- CRITICAL FIX: Calculate current_qty directly from transactions, not from corrupted satguru_stock
    ...
    CASE 
        ...
        WHEN (COALESCE(opening.opening_stock, 0) + COALESCE(grns.total_grns, 0) - COALESCE(issues.total_issues, 0)) <= COALESCE(im.reorder_level, 0) THEN 'low_stock'
        WHEN (COALESCE(opening.opening_stock, 0) + COALESCE(grns.total_grns, 0) - COALESCE(issues.total_issues, 0)) > (COALESCE(im.reorder_level, 0) * 3) THEN 'overstock'
        ELSE 'normal'
    END as stock_status
...
-- Verification: Check LDPELAM_MILKY_815_25 shows +358 now
SELECT 
    'CORRECTED_VERIFICATION' as status,
    item_code,
    current_qty,
    opening_stock,
    total_grns,
    total_issues,
    (opening_stock + total_grns - total_issues) as manual_calculation,
    stock_status
FROM satguru_stock_summary_view 
WHERE item_code = 'LDPELAM_MILKY_815_25';

-- Verify negative stock count
SELECT 
    'FINAL_NEGATIVE_CHECK' as status,
    COUNT(*) as negative_items_remaining
FROM satguru_stock_summary_view 
WHERE current_qty < 0;