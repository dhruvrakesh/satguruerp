...
-- Create test orders with today's date using correct column structure
INSERT INTO public.orders_dashboard_se (uiorn, item_code, item_name, date, created_at, last_activity) VALUES
('260125_0001', 'ITM-IKRISH-001', 'WRAPPER COOL MINT 100G', CURRENT_DATE, now(), 'Order created for testing'),
('260125_0002', 'ITM-SRS-002', 'Jimny-Lime&Aloevera', CURRENT_DATE, now(), 'Order created for testing'),
('260125_0003', 'ITM-GLOB-002', 'Soap-Rose', CURRENT_DATE, now(), 'Order created for testing')
...
-- Generate realistic color measurements for testing
...
measurement_series AS (
  SELECT 
    s.session_id,
    s.target_l + (random() * 4 - 2) as measured_l,  -- ±2 variation
    s.target_a + (random() * 3 - 1.5) as measured_a, -- ±1.5 variation  
    s.target_b + (random() * 3 - 1.5) as measured_b, -- ±1.5 variation
    s.uiorn,
    generate_series(1, 
      CASE 
        WHEN s.uiorn = '260125_0001' THEN 15  -- Active session with many measurements
        WHEN s.uiorn = '260125_0002' THEN 8   -- Active session with fewer measurements
        WHEN s.uiorn = '260125_0003' THEN 25  -- Completed session with full dataset
      END
    ) as measurement_num
  FROM session_data s
)
