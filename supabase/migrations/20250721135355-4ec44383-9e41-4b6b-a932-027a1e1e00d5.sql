-- EMERGENCY STOCK CALCULATION RECOVERY PLAN - FIXED
-- Phase 1: Fix the broken calculate_current_stock function and rebuild infrastructure
...
-- Rebuild the stock summary view with proper data (without reorder_level)
...
SELECT 
    ...
    im.category_id,
    0 as reorder_level, -- Default to 0 since column doesn't exist
    ...
    CASE 
        ...
        WHEN s.current_qty <= 10 THEN 'low_stock'  -- Fixed threshold since no reorder_level
...
BEGIN
    ...
    SELECT EXISTS(SELECT 1 FROM satguru_item_master WHERE item_code = p_item_code) INTO v_item_exists;
    ...
    -- Validate issue transactions
    IF p_transaction_type = 'ISSUE' AND v_current_stock < p_quantity THEN
        RETURN jsonb_build_object(
            'valid', false,
