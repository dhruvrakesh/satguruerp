...
export function ItemMasterCleanup() {
  ...
  const findDuplicates = async () => {
    setIsLoading(true);
    try {
      // Get all items with their creation dates from satguru_item_master
      ...
      for (const [itemName, items] of Object.entries(groups || {})) {
        ...
        if (itemList.length > 1) {
          ...
          // Check which items to remove have stock references in satguru_stock
  ...
  const transferStockData = async (fromItemCode: string, toItemCode: string) => {
    ...
    // Update satguru log tables (safe to do always)
    await supabase
        ...
        try {
          ...
          for (const itemToRemove of group.items_to_remove) {
            try {
              ...
              // Now safe to delete the duplicate item from satguru_item_master
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Satguru Item Master Cleanup - Duplicate Names
        </CardTitle>
        <CardDescription>
          Find duplicate item names and keep the oldest item_code while safely transferring all related data from Satguru tables
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {duplicates.length > 0 && (
          <Alert>
            ...
            <AlertDescription>
              ...
              Strategy: Keep oldest item_code for each name, transfer stock data safely from Satguru tables, then delete duplicates.
            </AlertDescription>
          </Alert>
        )}
        ...
        {duplicates.length === 0 && !isLoading && (
          <Alert>
            ...
            <AlertDescription>
              No duplicate item names found in Satguru item master data. Your data is clean!
            </AlertDescription>
          </Alert>
        )}
      </CardContent>
