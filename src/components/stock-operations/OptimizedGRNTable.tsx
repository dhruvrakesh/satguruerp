...
import { useOptimizedGRN, useGRNMutations, GRNSort, GRNFilters } from "@/hooks/useOptimizedGRN";
...
interface OptimizedGRNTableProps {
  onEdit?: (grn: GRNRecord) => void;
}

export function OptimizedGRNTable({ onEdit }: OptimizedGRNTableProps) {
  ...
  const { data, isLoading, error, isFetching } = useOptimizedGRN({ 
    page, 
    pageSize: 50, 
    filters, 
    sort 
  ...
  const handleEdit = useCallback((id: string, currentValues: GRNRecord) => {
    setEditingId(id);
