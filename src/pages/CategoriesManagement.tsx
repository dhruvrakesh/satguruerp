...
import { useCategoriesWithStats, useCategoryMutations, CategoryWithStats } from "@/hooks/useCategories";
...
export default function CategoriesManagement() {
  const { data: categoryStats, isLoading } = useCategoriesWithStats();
  const { createCategory, updateCategory, deleteCategory } = useCategoryMutations();
  const [isCreateDialogOpen, setIsCreateDialogOpen] = useState(false);
  const [editingCategory, setEditingCategory] = useState<CategoryWithStats | null>(null);
  const [formData, setFormData] = useState<CategoryFormData>({ category_name: "", description: "" });

  // Ensure categoryStats is properly typed as an array
  const categories = (categoryStats as CategoryWithStats[]) || [];
  ...
  const getUsageTypeStats = (category: CategoryWithStats) => {
    const stats = [
              ...
              {categories.length === 0 ? (
                ...
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center p-4 border rounded-lg">
                      <div className="text-2xl font-bold text-primary">
                        {categories.reduce((sum: number, cat: CategoryWithStats) => sum + cat.fg_items, 0)}
                      </div>
                      <div className="text-sm text-muted-foreground">Finished Goods</div>
                    </div>
                    <div className="text-center p-4 border rounded-lg">
                      <div className="text-2xl font-bold text-secondary">
                        {categories.reduce((sum: number, cat: CategoryWithStats) => sum + cat.rm_items, 0)}
                      </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {categories.slice(0, 5).map((category: CategoryWithStats) => (
